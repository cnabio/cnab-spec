...
The following code listing is an example of the `example-image-developers` metadata:
  ...
  "signed": {
    ...
    "delegations": {
      ...
      "roles": [
        {
          "keyids": [...],
          "name": "example-image-automation",
          "paths": [
            "example.com/example-org/example-image:*",
            "example.com/example-org/example-image/in-toto-metadata/*/*.link"
          ],
          "terminating": true,
          "threshold": 1
        }
    ...
    "targets": {
      "example-image/in-toto-metadata/root.layout": {
        "hashes": {
      ...
      "example-image/in-toto-pubkeys/298f37401f0b526a708967b7f708bc9c938fe0ad4bfe50d66837c20a57084e84.pub": {
        "hashes": {
      ...
      "example-image/in-toto-pubkeys/3e82bcdc71b29999340ceaadf3dc4193f8b06572d1c20612e9acdd7b52fa4b90.pub": {
        "hashes": {
      ...
      "example-image/in-toto-pubkeys/e847f58ca5e83fc48d1d2388ddd8f1a168b205a3fe7978ad015dee3ae7b2ecf7.pub": {
        "hashes": {
...
Finally, the CI/CD automation (e.g., `example-image-automation`) SHOULD sign targets metadata (e.g., cryptographic hashes and file sizes) for all available tags.

If end-to-end authenticity and integrity of tags is desired, each tag (e.g. `latest`) MAY include custom targets metadata that refers to a complete set of in-toto link metadata (a.k.a. _links_) required to verify that tag according to the root layout specified by the image developers (e.g., `example-image-developers`). In this case, the automation MAY also sign targets metadata about all links for all available tags.

For each tag, there MAY be as many links as there are steps in the root layout. The exact setup of the root layout depends on the organization and the specific group of developers, and hence out of the scope of this document. However, as a working example, suppose that there are two steps in the root layout:

1. In the `developer` step, developers MAY sign link metadata about all code and data required to build a tag. They MAY use offline keys (e.g. [YubiKeys](https://github.com/DataDog/yubikey)) to sign this link. Finally, they MAY also check all of this code and data into a version control system (VCS).
2. In the `builder` step, the CI/CD automation MAY sign link metadata about all of the code and data it received from this VCS about this tag. It MAY also sign link metadata about the tag it built using this code and data. (As part of [reproducible builds](https://reproducible-builds.org), which is out of the scope of this document, the root layout MAY require two different builders to agree on the same digest for the same tag.) The automation MAY use online keys to sign this link. In fact, it MAY even share the same online key with the `timestamp` and `snapshot` roles.

In order to prevent conflicts between links for different tags, the complete set of link metadata for each tag MAY be isolated in different directories. The simplest way to do this is to use a separate directory for each tag that is named after the digest (e.g., SHA-256) for that tag.

Returning to the TUF targets metadata for the automation, it SHOULD use a threshold (1, 1) of online keys. In fact, it MAY even share the same online key with the `timestamp` and `snapshot` roles. Its metadata SHOULD expire weekly, assuming that new tags are produced a few times a week.

The following code listing is an example of the `example-image-automation` metadata:
