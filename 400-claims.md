...
- `result` (REQUIRED): The outcome of the bundle's last action (e.g. if action is install, this indicates the outcome of the installation.). It is an object with the following fields:
  ...
  - `status` (REQUIRED): Indicates the status of the last phase transition. Valid statuses are:
    ...
    - `running`: Execution is in progress and has not yet completed.
...
The claim `result` provies a list of output names generated by the operation. A tool may choose to request the contents of any outputs to persist them but is not required to do so.
...
Below, you can see an example of a claim result that included an output file, `clientCert`.
